...
export default function TunnelGLSLVisualization() {
  ...
  useEffect(() => {
    ...
    // Tunnel/Vortex fragment shader with audio reactivity
  ...
  void main() {
    ...
    float tunnelSpeed = u_time * (0.5 + u_audioLevel * 0.8 * u_intensity);
    float rotationAngle = u_time * u_rotationSpeed * (1.0 + u_bassLevel * 0.5);
    ...
    float depth = tunnelSpeed + radius * (2.0 + u_audioLevel * 2.0 * u_scaleReactivity);
    
    // Create tunnel effect with audio-reactive rings
    float rings = sin(depth * (8.0 + u_bassLevel * 4.0 * u_fractalComplexity)) * 0.5 + 0.5;
    float spiral = sin(angle * (6.0 + u_midLevel * 4.0) + depth * (2.0 + u_trebleLevel * 2.0)) * 0.5 + 0.5;
    
    // Frequency-based ring modulation
    float freqPos = mod(depth * 0.1, 1.0);
    float freqIntensity = getFrequency(freqPos);
    rings *= 1.0 + freqIntensity * 0.5 * u_intensity;
    
    // Combine effects
    float pattern = rings * spiral;
    
    // Audio-reactive radial waves
    float radialWaves = sin(radius * (15.0 + u_audioLevel * 10.0) - tunnelSpeed * 3.0) * 0.5 + 0.5;
    pattern += radialWaves * u_audioLevel * 0.3;
    
    // Beat-reactive pulses
    float beatPulse = 1.0 + u_bassLevel * u_bassLevel * u_beatPulse;
    pattern *= beatPulse;
    
    // Dynamic color based on position and audio
    float hue1 = angle / (2.0 * 3.14159) + u_time * 0.1 + u_bassLevel * 0.2 * u_colorSensitivity;
    float hue2 = depth * 0.05 + u_midLevel * 0.3 * u_colorSensitivity;
    float hue3 = radius + u_trebleLevel * 0.4 * u_colorSensitivity;
    
    // Blend hues based on audio characteristics
    float hue = mix(mix(hue1, hue2, u_midLevel * 0.6), hue3, u_trebleLevel * 0.4);
    
    // Audio-reactive saturation and brightness
    float saturation = 0.8 + u_audioLevel * 0.2 * u_colorSensitivity;
    float brightness = pattern * (0.8 + u_audioLevel * 0.4 * u_intensity);
    
    // Frequency-based brightness modulation
    brightness += freqIntensity * 0.2 * u_colorSensitivity;
    
    // Edge glow effect
    float edgeGlow = 1.0 / (1.0 + radius * radius * (5.0 - u_audioLevel * 3.0));
    brightness += edgeGlow * u_audioLevel * 0.3;
    
    vec3 color = hsv(hue, saturation, brightness);
    
    // Add waveform visualization as particles
    float wavePos = angle / (2.0 * 3.14159) * 0.5 + 0.5;
    int waveIdx = int(wavePos * 31.0);
    float waveValue = u_waveformData[waveIdx];
    
    if (abs(radius - 0.7) < 0.05 + abs(waveValue) * 0.1) {
      color += vec3(0.3, 0.5, 1.0) * abs(waveValue) * 2.0;
    }
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: "15px", borderTop: "1px solid rgba(255, 255, 255, 0.2)", paddingTop: "15px" }}>
          ...
          {/* Ring Complexity (renamed from Fractal Complexity) */}
          <div style={{ marginBottom: "10px" }}>
            <label style={{ display: "block", marginBottom: "3px", fontSize: "11px", opacity: 0.8 }}>
              Ring Complexity: {Math.round(fractalComplexity * 100)}%
            </label>
            <input
              type="range"
              min="0"
